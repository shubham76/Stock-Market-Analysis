...
for k,v in od.items():
    print v
    date1 = datetime.datetime.strptime(str(k), '%m/%d/%Y').strftime('%Y-%m-%d')
    temp=pd.DataFrame({'open_score':[open_score[k]],'close_score':[close_score[k]],'date':date1}) #'index':[c],
    #c += 1
    result = pd.concat([result,temp])

result = result.set_index('date')
print(result)
result.to_csv('TCS.csv',sep=',',encoding='utf-8')
